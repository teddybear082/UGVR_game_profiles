...
# Custom variables for awoken
var awoken_underwater = null
var awoken_slowmoShader = null
var awoken_berserkShader = null
...
func _process(delta):
	...
	if not is_instance_valid(awoken_slowmoShader):
		...
		if awoken_slowmoShader:
			remove_shader_from_UI_object(awoken_slowmoShader)
	if not is_instance_valid(awoken_underwater):
		awoken_underwater = find_first_node_by_name("underwater")
		if awoken_underwater:
			remove_shader_from_UI_object(awoken_underwater)
	if not is_instance_valid(awoken_electrifiedShader):
		awoken_electrifiedShader = find_first_node_by_name("electrifiedShader")
		if awoken_electrifiedShader:
			remove_shader_from_UI_object(awoken_electrifiedShader)
	if not is_instance_valid(awoken_berserkShader):
		awoken_berserkShader = find_first_node_by_name("berserkShader")
		if awoken_berserkShader:
			remove_shader_from_UI_object(awoken_berserkShader)
	if not is_instance_valid(awoken_acid):
		awoken_acid = find_first_node_by_name("acid")
		if awoken_acid:
			remove_shader_from_UI_object(awoken_acid)
	if not is_instance_valid(awoken_lava):
		awoken_lava = find_first_node_by_name("lava")
		if awoken_lava:
			remove_shader_from_UI_object(awoken_lava)
