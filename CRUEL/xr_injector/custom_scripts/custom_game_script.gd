...
func _process(delta):
	...
	if is_instance_valid(xr_scene.xr_roomscale_controller):
		if is_instance_valid(xr_scene.xr_roomscale_controller.current_characterbody3D):
			...
			if CRUEL_pistol_parent != null and xr_scene.xr_origin_reparented:
				...
				if first_time_reparenting_CRUEL_gun or not is_instance_valid(CRUEL_camera_node):
					...
					var cursor = xr_scene.cursor_3d.duplicate()
